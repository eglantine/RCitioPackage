...
getFileFromCloud = function(base_url, session_id, path){
  gcloud_base_url =  paste0(base_url, "/rest/gcloud/blob?path=")

  full_query = paste0(gcloud_base_url,path)
  df = getResponseFromRoute(full_query, session_id)
}

getFileListFromCloud = function(base_url, session_id, path){
  file_list = getFileList(base_url, session_id, path)

  i = 1
  data = data.frame()

  for(i in 1:file_list){
